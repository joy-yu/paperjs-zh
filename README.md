...
* 很好地解释了矢量图形中的矢量。Paper.js 将矢量数学视为一等公民，它通过其核心类型（如Point、Size和Rectangle）尽可能简单地处理矢量和几何图形。PaperScript 进一步简化了 Point 和 Size 对象的操作，使用普通操作符语法的直接数学运算可以在这些对象上进行，就像它们是纯数字一样。The manipulation of Point and Size objects is further simplified in PaperScript, where direct math operations using normal operator syntax are possible on such objects as if they were plain numbers.
